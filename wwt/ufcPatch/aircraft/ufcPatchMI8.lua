...
function ufcPatchMI8.generateUFCData()
	...
	local FC3RadarAlt = {
	math.floor(LoGetAltitudeAboveGroundLevel())-2
	} -- -2 accounts for airframe offset
	...
	--Heading 
    local Headingdigits =  {math.floor(LoGetMagneticYaw()* (180/math.pi))} --{math.floor(MainPanel:get_argument_value(25) * 360)}
	...
	--Airspeed
	local Airspeeddigits = {math.floor(LoGetIndicatedAirSpeed()*3.6)}  --Convert Meters per Second to Kph --{math.floor(MainPanel:get_argument_value(24) * 10)}
	...
	--Fuel (Internal)	
	local Fueldigits = {math.floor((LoGetEngineInfo().fuel_internal)*2585)} --Coverts % fuel to liters (Uses 2585 as 100% per Chuck's Guide)   --{math.floor(MainPanel:get_argument_value(62) * 2800)} --Non-linear output, needs more work
...
--Radios
	...
	--ARK 9 
	local ARK9BDialdigits = {math.floor((MainPanel:get_argument_value(675) * 21)+1), --Add since 0 is Channel 1, number for multiplication due to Arg rounding requirements 
	...
	local ARK9PDialdigits = {math.floor((MainPanel:get_argument_value(678) * 21)+1), --Add since 0 is Channel 1, number for multiplication due to Arg rounding requirements 
